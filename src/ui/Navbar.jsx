  ...
  return (
    <motion.nav
      className={classNames(
        "fixed top-0 right-0 left-0 z-49 w-full transition-all duration-500 ease-in-out hover:cursor-none",
        scrolled
          ? "bg-white/90 py-2 shadow-sm backdrop-blur-lg"
          : "bg-transparent py-0",
      )}
    >
      <div className="mx-auto flex h-20 max-w-7xl items-center justify-between px-6 md:px-12">
        ...
        <div className="hidden items-center gap-10 lg:flex xl:gap-14">
          {navLinksItems.map((link) => (
            <NavLink
              ...
              className={({ isActive }) =>
                classNames(
                  "font-quicksand group relative py-1 text-[0.95rem] font-medium tracking-wide transition-colors duration-300 hover:cursor-none xl:text-base",
                  {
        ...
        {/* Mobile hamburger */}
        <button
          className={classNames("z-50 lg:hidden", isOpen && "hidden")}
        ...
        <AnimatePresence>
          {isOpen && (
            <motion.div
              className="fixed inset-0 z-50 flex flex-col bg-gray-950 lg:hidden"
            ...
            >
              ...
              <motion.div
                className="flex flex-col items-center gap-5 px-8 pb-10"
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
